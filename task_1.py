...
class OnlineSalesRegisterCollector:
    ...
    def __discount_factor(self) -> float:
        return 0.9 if self.__number_items > 10 else 1.0

    def check_amount(self) -> float:
        total = [self.__item_price[item] for item in self.__name_items]
        return sum(total) * self.__discount_factor()
    ...
    def twenty_percent_tax_calculation(self) -> float:
        ...
        return sum(total) * self.__discount_factor() * 0.2
    ...
    def ten_percent_tax_calculation(self) -> float:
        ...
        return sum(total) * self.__discount_factor() * 0.1
